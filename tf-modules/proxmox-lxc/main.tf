...
resource "proxmox_virtual_environment_container" "this" {
  ...
  lifecycle {
    ...
    #
    # `features` updates require root@pam on privileged containers; our API
    # token (terraform@pve) can't change them. For existing containers, the
    # features already match reality — the diff is bpg surfacing default
    # fields that were implicit in the previous import. Ignoring keeps
    # apply working with a non-root token; greenfield create still honors
    # the features block on initial provision.
    ignore_changes = [
