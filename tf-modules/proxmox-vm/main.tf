...
resource "proxmox_virtual_environment_vm" "this" {
  ...
  lifecycle {
    ...
    #
    # `clone` is a create-time field (used only at clone-from-template time)
    # but bpg/proxmox surfaces it as a diff on imported VMs because Proxmox
    # doesn't track clone lineage post-creation. Ignoring it lets greenfield
    # clones still work while imported VMs don't see spurious replace plans.
    #
    # `initialization[0].user_data_file_id` is wired to a sibling snippet
    # resource that we may re-create on each apply (snippets aren't always
    # importable). The actual cloud-init data was consumed at boot, so the
    # ID drifting in state has no runtime effect.
    ignore_changes = [
      mac_addresses,
      clone,
