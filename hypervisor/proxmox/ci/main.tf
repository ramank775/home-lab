locals {
  versions = {
    ...
    forgejo_runner = "v6.3.1"
  }

  # Internal metallb VIP for forgejo (var.code.server_ip in deployment tree).
  # Hardcoded here to keep hypervisor/ and deployment/ trees independent.
  forgejo_instance_url = "http://10.0.0.55:3000"
...
resource "proxmox_virtual_environment_file" "forgejo_runner_user_data" {
  ...
  source_raw {
    ...
    data = templatefile("${path.module}/cloud-init/forgejo-runner.yaml.tftpl", {
      ...
      runner_version     = local.versions.forgejo_runner
